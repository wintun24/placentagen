...
def smoothing(rectangular_mesh,el_value):
    """
    this subroutine is used to smooth the sampling grid element value. 
    el_value can be any samp_grid parameter that needs smoothing. (e.g.tissue vol of sampling grid)
    For smoothing, each sampling grid element value is assinged to its node. Then, the nodes that are shared by multiply sampling grid 
    are calculated by averaging all differnt node value. 
    Finally, the element value is calculated back by averaging its own node value (this node value is after averaging). 

    """

    total_sample_elems = rectangular_mesh['total_elems']
    elems = rectangular_mesh['elems']
    nnode= rectangular_mesh['total_nodes']
    node_per_samp_grid=8
    node_value = np.zeros((nnode, 2),dtype=object)
    node_value[:,0]=float(0)
    node_value[:,1]=int(0)
    
    for i in range(0,len(elems)):
       for j in range(1, len(elems[i])):
          node_value[elems[i,j],1]=node_value[elems[i,j],1]+1#adding up the number if the nodes are shared between sampling grids
          
          node_value[elems[i,j],0]=(node_value[elems[i,j],0]*(node_value[elems[i,j],1]-1)+el_value[i])/node_value[elems[i,j],1]#calculate the average value of nodes
           
    
    smooth_el_value=np.zeros(total_sample_elems)
    
    for ne in range(0,total_sample_elems): 
    
        temp_smooth_el=np.array([[node_value[elems[ne,1],0],node_value[elems[ne,2],0] , node_value[elems[ne,3],0], node_value[elems[ne,4],0],node_value[elems[ne,5],0], node_value[elems[ne,6],0] , node_value[elems[ne,7],0], node_value[elems[ne,8],0]]])
        
        smooth_el_value[ne]=np.sum(temp_smooth_el)/node_per_samp_grid
    
    
    return{'node_value': node_value,'smooth_el_value':smooth_el_value}
