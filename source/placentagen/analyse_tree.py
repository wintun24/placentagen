...
def cal_br_vol_samp_grid(rectangular_mesh,eldata,nodedata,volume,thickness,ellipticity,p_vol):
    ...
    4. calculate the volume of individual branch for all branches in the whole tree
    ...
    master_vol_samp_gr=np.zeros((total_elems, 2))#1st col of stores total vol of braches in each samp_grid el, 2nd col stores diameter variable of branches in each samp_grid el
    b_count=0
    ...
    for ne in range (0,len(branch_el)):#looping for all branchs in tree
       ...
       yp=np.linspace(-r,r,points)#linspace along y axis of cross-section of br
       zp=np.linspace(-r,r,points)#linspace along z axis of cross-section of br
       [yd,zd]=np.meshgrid(yp,zp)#generate meshgrid on yz plane
       ...
       dp_cross_section=dp_cross_section[:,0:counter]#yz coor for one layer
       dp_cross_section=np.matlib.repmat(dp_cross_section[:],1,len(dp_along_length)) #repeating yz coor for all layers along length
       dp_whole_br=np.matlib.repmat(dp_along_length[0], 1, counter)#create x vector /x coor for one layer
       
       for c in range (1,len(dp_along_length)):#x coor for all layer
       ...
       datapoints = dp_whole_br_new.T# adjusted coor of datapoints in one branch element
       ...
       master_vol_samp_gr[:,0] = master_vol_samp_gr[:,0] + vol_samp_gr# adding up volume  as one samp_grid_el may have more than one branch element
       master_vol_samp_gr[:,1] = master_vol_samp_gr[:,1] + vol_samp_gr*r*2;#adding up diam related variable
    

    pl_vol_in_grid=p_vol['pl_vol_in_grid']
    for i in range (0,len(master_vol_samp_gr)):#countercheck
         if pl_vol_in_grid[i]==0 and master_vol_samp_gr[i,0]!=0 :#this should not happen
    ...
    return{'master_vol_samp_gr': master_vol_samp_gr, 'br_num_in_samp_gr':br_num_in_samp_gr,'vol_each_br':vol_each_br,'total_br_vol':np.sum(vol_each_br)}
